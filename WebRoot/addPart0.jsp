...
<html>
  ...
  <body>
    ...
    <div id="result" align="center" style="background:#B5DBFF;min-height:600px">
      ...
      <table id = "tablePart" width = "800">
        <tr>
          <th>样品编号</th><th>样品名称</th><th>部件名称</th><th>数量</th><th>标准编号</th><th><button type="button" onclick="savePart()">保存部件</button></th><th><button type="button" >显示部件</button></th>
        </tr>
        <tr>
          <td><script>document.write(requestURLParameter('no'));</script></td>
          <td><script>document.write(requestURLParameter('specimenName'));</script></td>
          <td><input type="text" id="partName" style="width:100px"/></td>
          <td><input type="text" id="partNo" style="width:50px"/></td>
          <td><select><script>document.write(selectFunction());</script></select></td>
          <td><input type="button" class="addTr"  value = "添加部件" /></td>
          <td><input type="button" class="deleteTr"  value = "删除部件" onclick="deleteTr()"/></td>
        </tr>
        <tr>
          <th>样品编号</th><th>样品名称</th><th>部件名称</th><th>数量</th><th>标准编号</th><th><button type="button" onclick="savePart()">保存部件</button></th><th><button type="button" class = "showPart" onclick="showPart()">显示部件</button></th>
        </tr>
      </table>
    </div>
  </div>
